...
const loginUser = async ({ username, password }) => {
  ...
  if (!correctpw) throw new Error("Invalid username or password")
  ...
  return { token: jwtToken, user: { username: userdb.username, email: userdb.email, id: userdb.id } }
}
...
const googleAuth = async (token) => {
  ...
  if (!userdb) userdb = await db.User.create({ email, username: sub, password: "GoogleAuth-" + name })
